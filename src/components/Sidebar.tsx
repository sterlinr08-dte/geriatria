...
import { Egg, LogOut, X } from 'lucide-react'
import { useAuth } from '../lib/auth'
import { AVICOLA_GROUPS, AVICOLA_MODULES } from '../core/modules'

interface Props { open: boolean; onClose: () => void }

export default function Sidebar({ open, onClose }: Props) {
  const { perfil, signOut, puede } = useAuth()
  const gruposVisibles = AVICOLA_GROUPS
    .map((titulo) => ({ titulo, links: AVICOLA_MODULES.filter((item) => item.group === titulo && puede(item.key)) }))
    ...
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-[270px] transform flex-col bg-[#123A25] text-white shadow-2xl transition-transform duration-200 lg:static lg:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex h-16 items-center gap-3 border-b border-white/10 px-5">
          ...
          <div><p className="text-[17px] font-bold tracking-tight">AVÍCOLA ERP</p><p className="text-[10px] uppercase tracking-[0.18em] text-emerald-200">Enterprise 2026</p></div>
          ...
          {gruposVisibles.map((grupo) => (
            <section key={grupo.titulo}>
              ...
              <div className="space-y-1">
                {grupo.links.map(({ path, label, icon: Icon, end }) => (
                  <NavLink key={path} to={path} end={end} onClick={onClose} className={({ isActive }) => `flex items-center gap-3 rounded-xl px-3 py-2.5 text-[13px] font-medium transition ${isActive ? 'bg-white text-[#1F6A34] shadow-sm' : 'text-emerald-50/85 hover:bg-white/10 hover:text-white'}`}>
        ...
        <div className="border-t border-white/10 p-3">
          <div className="mb-2 rounded-xl bg-white/7 px-3 py-2.5"><p className="truncate text-xs font-semibold">{perfil?.nombre || perfil?.username || 'Administrador'}</p><p className="mt-0.5 text-[11px] text-emerald-200/70">{perfil?.rol_nombre || 'Granja Principal'}</p></div>

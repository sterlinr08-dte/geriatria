...
type Row = {
  ...
  aves: number
  postura: number
  estado: 'Producción' | 'Desarrollo' | 'Alerta'
}
...
const rows: Row[] = Array.from({ length: 50 }, (_, index) => ({
  ...
  descripcion: index === 2
    ? 'Lote con observación operativa extensa: requiere seguimiento de postura, ventilación, consumo y comportamiento durante la transición para comprobar que el contenido largo se ajuste sin romper la tabla.'
    : 'Seguimiento productivo estándar del lote.',
  aves: 8200 + index * 37,
...
export default function DesignSystemPreview() {
  ...
  const pageSize = 8
  const filtered = useMemo(() => rows.filter((row) => `${row.lote} ${row.galpon} ${row.descripcion}`.toLowerCase().includes(search.toLowerCase())), [search])
  const visible = mode === 'normal' ? filtered.slice(page * pageSize, page * pageSize + pageSize) : []
  const displayedTotal = mode === 'empty' ? 0 : filtered.length
  const displayedPages = mode === 'empty' ? 1 : Math.max(1, Math.ceil(filtered.length / pageSize))
  ...
  const columns: DataColumn<Row>[] = [
    ...
    { key: 'descripcion', header: 'Observación', className: 'min-w-[320px] max-w-xl', render: (row) => <p className="whitespace-normal leading-5 text-slate-600">{row.descripcion}</p> },
  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <KpiCard label="Inventario" value="9,035" icon={Package} helper="bandejas disponibles, sin tendencia calculada" />
    </section>
    ...
    <div className="flex flex-wrap gap-2">
      ...
      <button className={mode === 'error' ? 'btn-primary' : 'btn-ghost'} onClick={() => { setMode('error'); setPage(0) }}>Estado de error</button>
    </div>

    <DataTable rows={visible} columns={columns} getRowKey={(row) => row.id} page={page} totalPages={displayedPages} totalRecords={displayedTotal} onPageChange={setPage} searchValue={search} onSearchChange={(value) => { setSearch(value); setPage(0) }} searchPlaceholder="Buscar lote, galpón u observación" error={mode === 'error' ? 'No fue posible consultar los lotes. Revisa la conexión e inténtalo nuevamente.' : null} emptyTitle="No hay lotes para mostrar" emptyDescription="Prueba otro término o registra el primer lote productivo." />

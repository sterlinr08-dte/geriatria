...
export default function KpiCard({ label, value, icon: Icon, helper, trend, suffix }: Props) {
  ...
  const trendClass = trend === undefined || trend === 0 ? 'text-slate-500' : trend > 0 ? 'text-emerald-700' : 'text-rose-700'
  return (
    <article className="rounded-[14px] border border-slate-200/80 bg-white p-4 shadow-[0_10px_30px_-24px_rgba(15,23,42,.45)]">
      ...
      {(helper || trend !== undefined) && <div className="mt-3 flex items-center gap-2 border-t border-slate-100 pt-3 text-xs"><span className={`inline-flex items-center gap-1 font-semibold ${trendClass}`}><TrendIcon size={14} aria-hidden="true" />{trend !== undefined ? `${Math.abs(trend).toFixed(1)}%` : ''}</span>{helper && <span className="truncate text-slate-500">{helper}</span>}</div>}
    </article>
  )
}
